...
export var Lunar = {
  ...
  isHoliday: function (year, month, day) {
    var lunarHoliday = ['正月-初一', '五月-初五', '八月-十五']
    var lunarDay = this.toLunar(year, month, day)
    var temp = lunarDay[5] + '-' + lunarDay[6]
    return lunarHoliday.indexOf(temp) >= 0
  },
  getHoliday: function (year, month, day) {
    var lunarHoliday = [
      { name: '春节', value: '正月-初一' },
      { name: '端午节', value: '五月-初五' },
      { name: '中秋节', value: '八月-十五' },
    ]
    var lunarDate = this.toLunar(year, month, day) || ''
    var temp = lunarDate[5] + '-' + lunarDate[6]
    for (var i = 0; i < lunarHoliday.length; i++) {
      var item = lunarHoliday[i]
      if (temp == item.value) {
        return item.name
      }
    }
...
export function getCurrencyFormat(val){
  var n = val || 0
  if (isNaN(val)) {
    n = 0
  }
