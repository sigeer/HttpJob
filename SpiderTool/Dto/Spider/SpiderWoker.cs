...
namespace SpiderTool.Dto.Spider
{
    public class SpiderWorker
    {
        ...
        public async Task Process()
        {
            ...
            if (Spider.Method == RequestMethod.POST)
                documentContent = await HttpRequest.PostAsync(_currentUrl!.GetTotalUrl(HostUrl), Spider.PostObj, Spider.GetHeaders());
            else
                documentContent = await HttpRequest.GetAsync(_currentUrl!.GetTotalUrl(HostUrl), Spider.GetHeaders());

            _currentDoc.LoadHtml(documentContent);
            await ExtractContent();
            await MoveToNextPage();
            Console.WriteLine("====开始合并====");
            CombineText(CurrentDir);
        ...
        private async Task ExtractContent()
        {
            foreach (var rule in Spider.TemplateList)
            {
                if (rule.Type == (int)ContentType.DownloadLink)
                {
                    ...
                    {
                        var resource = (item.Attributes["src"] ?? item.Attributes["data-src"]).Value;
                        await SpiderUtility.SaveUrlResource(CurrentDir, resource.GetTotalUrl(HostUrl));
                    }
                }
                ...
                if (rule.Type == (int)ContentType.Text)
                {
                    var finalText = _currentDoc.Text;
                    SpiderUtility.SaveText(CurrentDir, _currentDoc.Text);
                }
        ...
        public void CombineText(string dir)
        {
    ...
    {
        public static readonly string[] InvalidFolderSymbol = new string[] { "\\", "/", "|", ":", "*", "^", "<", ">", "\"" };
        ...
        public static string GetText(this string str)
        {
            return HttpUtility.HtmlDecode(str);
        }

        public static async Task SaveUrlResource(string dir, string url)
        {
            try
            {
                var uri = new Uri(url);
                var path = Path.Combine(dir.GetDirectory(), uri.Segments.Last());
                Console.WriteLine("File Save : " + path);
                var result = await HttpRequest.GetRaw(url);
                File.WriteAllBytes(path, await result.Content.ReadAsByteArrayAsync());
            }
            catch (Exception ex)
            {
